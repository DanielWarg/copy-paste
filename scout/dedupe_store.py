...
class DedupeStore:
    ...
    def is_seen(self, dedupe_key: str, max_age_hours: Optional[int] = None) -> bool:
        """
        Check if dedupe_key has been seen before.
        
        Args:
            dedupe_key: Dedupe key to check
            max_age_hours: If set, only consider items newer than this many hours as "seen".
                          Items older than max_age_hours will return False (can be polled again).
            
        Returns:
            True if seen (and within max_age_hours if specified), False otherwise
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        
        if max_age_hours is not None:
            # Only consider items newer than max_age_hours as "seen"
            cutoff_time = datetime.utcnow() - timedelta(hours=max_age_hours)
            cutoff_str = cutoff_time.isoformat()
            
            cursor.execute(
                "SELECT 1 FROM seen_items WHERE dedupe_key = ? AND detected_at >= ?",
                (dedupe_key, cutoff_str)
            )
        else:
            # Original behavior: check if exists at all
            cursor.execute(
