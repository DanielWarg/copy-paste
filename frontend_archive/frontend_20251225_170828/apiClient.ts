...
// I produktion via proxy: använd relativa paths (/api/...) eller https://localhost
// I dev: använd VITE_API_BASE_URL eller fallback till localhost:8000
// För mTLS (prod_brutal): använd https://localhost (proxy hanterar mTLS)
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 
  (import.meta.env.PROD ? 'https://localhost' : 'http://localhost:8000');
// Mock data endast om explicit satt till 'true' (inte default i DEV)
...
// Fetch wrapper med error handling och request correlation
async function apiFetch<T>(
  ...
  // Generate request ID for correlation (if not already provided)
  const outgoingRequestId = (options.headers as Record<string, string>)?.['X-Request-Id'] || generateRequestId();
  ...
  try {
    ...
    // Extract request_id from response header (backend echoes it back)
    ...
    if (!response.ok) {
      ...
      return {
        ...
        error: {
          ...
          request_id: errorData.request_id || responseRequestId,
        },
      };
    }
...
export const apiClient = {
  ...
  getTranscripts: async (): Promise<Transcript[]> => {
    ...
    if (result.error) {
      // No fallback to mock - return empty array on error (brutal-safe)
      console.warn('Failed to fetch transcripts:', result.error.code, result.error.request_id);
      return [];
    }
  ...
  getEvents: async (): Promise<NewsEvent[]> => {
    ...
    if (result.error) {
      // No fallback to mock - return empty array on error (brutal-safe)
      console.warn('Failed to fetch events:', result.error.code, result.error.request_id);
      return [];
    }
  ...
  getEventById: async (id: string): Promise<NewsEvent | undefined> => {
    ...
    if (result.error) {
      // No fallback to mock - return undefined on error (brutal-safe)
      console.warn('Failed to fetch event:', result.error.code, result.error.request_id);
      return undefined;
    }
  ...
  getSources: async (): Promise<Source[]> => {
    ...
    if (result.error) {
      // No fallback to mock - return empty array on error (brutal-safe)
      console.warn('Failed to fetch sources:', result.error.code, result.error.request_id);
      return [];
    }
   ...
   * @param text Raw text to generate draft from (will be masked via privacy_gate)
   * @param mode Privacy masking mode ("strict" | "balanced", default: "strict")
   */
  createDraft: async (eventId: number, text: string, mode: "strict" | "balanced" = "strict") => {
    if (USE_MOCK) {
      await delay(300);
      return {
        draft_id: 1,
        event_id: eventId,
        content: "[MOCK] Draft content for event " + eventId,
        created_at: new Date().toISOString()
      };
    }

    const result = await apiFetch<{ draft_id: number; event_id: number; content: string; created_at: string }>(
      `/api/v1/events/${eventId}/draft`,
      {
        method: "POST",
        body: JSON.stringify({
          raw_text: text,
    ...
    if (result.error) {
      // Handle PII detected (422) - show user-friendly message
      if (result.error.code === "pii_detected") {
        throw {
          code: "pii_detected",
          message: "PII detected – rensa texten",
