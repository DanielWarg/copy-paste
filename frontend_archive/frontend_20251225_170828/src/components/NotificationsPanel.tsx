...
export default function NotificationsPanel({ eventId }: NotificationsPanelProps) {
  ...
  const fetchStatus = async () => {
    ...
    setError(null)
    try {
      const response = await axios.get(`${SCOUT_URL}/scout/config/status`, getAxiosConfigWithRequestId())
  ...
  const handleTestNotification = async () => {
    ...
    try {
      const response = await axios.post(`${SCOUT_URL}/scout/notify`, { event_id: eventId }, getAxiosConfigWithRequestId())
      if (response.data.ok) {
