...
export default function FeedsPanel() {
  ...
  const handleToggleEnabled = async (feedId: string, enabled: boolean) => {
    try {
      await axios.patch(`${SCOUT_URL}/scout/feeds/${feedId}`, { enabled: !enabled }, getAxiosConfigWithRequestId())
      fetchFeeds()
    } catch (err: any) {
  ...
  const handleAddFeed = async () => {
    ...
    try {
      await axios.post(`${SCOUT_URL}/scout/feeds`, newFeed, getAxiosConfigWithRequestId())
  ...
  const handleDeleteFeed = async (feedId: string) => {
    ...
    try {
      await axios.delete(`${SCOUT_URL}/scout/feeds/${feedId}`, getAxiosConfigWithRequestId())
      fetchFeeds()
    } catch (err: any) {
  ...
  const handlePollNow = async (feedId: string) => {
    try {
      const response = await axios.post(`${SCOUT_URL}/scout/feeds/${feedId}/poll`, {}, getAxiosConfigWithRequestId())
