...
1. **Fail loud, fail early** - Konfigurationsfel och säkerhetskritiska invariants stoppar boot omedelbart
...
5. **Non-blocking migrations** - Migrationsfel stoppar inte startup (dev/prod policy kan variera)
...
**Process:**
1. Läser `.env` från working directory eller explicit `ENV_FILE` path (Pydantic Settings standard)
...
**Notera:** `.env`-sökvägen är relativ till Pydantic Settings-standard (working directory eller explicit `env_file` i model_config). I Docker/test kan `ENV_FILE` sättas explicit om sökvägen behöver variera.
...
3. Kör `Base.metadata.create_all()` (backup/fallback, Alembic hanterar migrations i produktion)
...
- **`create_all()` policy:** I utveckling kan detta vara ok som backup. I produktion bör schema styras enbart via Alembic migrations för att undvika schema drift och tysta mismatchar. Överväg att inaktivera `create_all()` i prod eller lägg till `MIGRATIONS_REQUIRED=true` policyflagga.
...
- **Produktionsrekommendation:** Överväg en policyflagga `MIGRATIONS_REQUIRED=true` som gör migrations blocking i prod. I dev är non-blocking praktiskt för lokal utveckling där DB kan vara i transit. I prod kan en app som startar med trasig schema vara sämre än att faila startup tidigt.
...
- Security headers sätts på **normala responses** (när middleware får en response från `call_next`)
- **Vid exceptions:** Om exception kastas inne i `call_next`, får middleware ingen response att sätta headers på. Exception handlers (som körs efter middleware) måste själva sätta security headers för konsekvent beteende, eller så används en reverse proxy (Caddy/Nginx) som sätter headers för alla responses.
...
class RequestIDMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        try:
            ...
            # Logga error (privacy-safe: exception class + type, inte str(e))
            # str(e) kan läcka paths, SQL-detaljer, nyckel-format, etc.
            logger.error("request_error", extra={
                "request_id": request_id,
                "path": str(request.url.path),
                "method": request.method,
                "error_type": type(e).__name__,  # Exception class name
                # I prod: logga endast exception-typ + eventuell error_code
                # I debug: kan str(e) vara tillåtet lokalt, men aldrig returneras till klient
            })
            raise
...
- **Security headers:** Sätts på normala responses när middleware får en response från handler. Vid exceptions skapas error responses av exception handlers (som körs efter middleware), så dessa måste själva sätta security headers för konsekvent beteende. Alternativt kan en reverse proxy (Caddy/Nginx) sätta headers för alla responses.
- **Privacy-safe logging:** Loggar endast metadata (exception class name, inte str(e)), inga bodies/headers. I prod: logga exception-typ + error_code. I debug: stacktrace kan visas lokalt, men aldrig returneras till klient.
...
3. **General Exception Handler:**
   ```python
   async def general_exception_handler(request, exc):
       # Logga exception (privacy-safe: exception class + error_code)
       logger.error("unhandled_exception", extra={
           "error_type": type(exc).__name__,  # Exception class, inte str(exc)
           # I prod: exception-typ + error_code endast
           # str(exc) kan läcka paths, SQL, nycklar
       })
       
       # Returnera generisk error (production)
       # ALDRIG stacktrace eller filvägar
       # Security headers måste sättas här (response skapas efter middleware)
   ```

**Viktiga detaljer:**

- **Request ID:** Alla errors inkluderar `request_id` från `request.state`
- **Production mode:** Generiska meddelanden, inga stacktraces, inga `str(e)` i logs
- **Debug mode:** Mer detaljer (men fortfarande inga headers/bodies/PII), stacktrace kan visas lokalt men aldrig returneras till klient
- **Privacy-safe:** Exception messages filtreras - logga endast exception class name + eventuell error_code, inte `str(e)` som kan läcka paths, SQL, nycklar
- **Security headers:** Exception handlers skapar nya JSONResponse efter middleware, så security headers måste sättas här för konsekvent beteende (eller använd reverse proxy)
...
- `str(exception)` direkt (kan läcka paths, SQL, nycklar) - logga endast exception class name + error_code
...
**Normala responses** inkluderar dessa headers (sätts av middleware):
...
**Error responses:** När exception handlers skapar nya JSONResponse efter middleware körts, måste de själva sätta security headers för konsekvent beteende. Alternativt kan en reverse proxy (Caddy/Nginx) sätta headers för alla responses (rekommenderat för produktion).
...
**Notera:** `/ready` använder `HTTPException` med `detail` dict, vilket ger response shape `{"detail": {...}}` istället för standard error-shape `{"error": {...}}`. Detta är ett undantag från konsekvent error-format för att behålla kompatibilitet med Kubernetes readiness probes. Överväg att uppdatera till standard error-shape i framtida versioner.
...
1. **Allowed Core Imports:**
   - Moduler får importera från `app.core.config` (Settings)
   - Moduler får importera från `app.core.logging` (logger)
   - Moduler får importera från `app.core.database` (Base, SessionLocal, get_db)
   - Moduler får importera från `app.core.privacy_guard` (sanitize_for_logging, assert_no_content, compute_integrity_hash)
   - Rationale: Dessa är stabila core-utilities som moduler behöver för DB-operationer och privacy-safe logging. De är inte interna CORE-detaljer utan del av det publika modul-kontraktet.
...
5. **Database Models:**
   ...
   - Använder `Base` från `app.core.database` (tillåtet import enligt Module Contract)
...
2. **Är privacy-safe** - Inga payloads, headers eller PII i logs, exception class name (inte str(e)) i prod
3. **Har konsekvent error handling** - Alla errors har samma shape med request_id (utom `/ready` 503 som använder `detail` format)
4. **Har security headers** - Sätts på normala responses via middleware, error responses behöver sätta headers i exception handlers eller via reverse proxy
